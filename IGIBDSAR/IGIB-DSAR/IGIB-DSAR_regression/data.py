...
def build_dataset(dataset, graph1, graph2, target):
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        delta_g = torch.tensor([dataset.loc[idx][target]], dtype=torch.float)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(f'/home/dwj/IGIBTPAMI/IGIB-ISE/IGIB-ISE_regression/data/raw_data/ZhangDDI_valid.csv', sep=",")
    # Chr : (Chromophore, Solvent, Absorption max (nm)/ Emission max (nm)/ Lifetime (ns))
    processed_data, dataset = build_dataset(df, "smiles_2", "smiles_1", "label")
    torch.save(processed_data, "./data/processed/ZvDDI.pt")